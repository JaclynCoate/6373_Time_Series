...
ARMA(5,1) Short Term
```{r}
shortARMA <- fore.aruma.wge(us$hospitalizedCurrently, phi = est.us.diff.seasAIC$phi, theta = est.us.diff.seasAIC$theta, d= 1, s = 7, n.ahead = 7, lastn = F, limits = F)
```

- ASE
```{r}
ASEshortARMA = mean((us$hospitalizedCurrently[(124-7+1):124]-shortARMA$f)^2)
ASEshortARMA 
```
...
- ASE
```{r}
ASElongARMA = mean((us$hospitalizedCurrently[(124-90+1):124]-longARMA$f)^2)
ASElongARMA 
```
...
** NEED TO FIGURE OUT WHY 90 DAY FORECAST ISN'T GRAPHING WINDOWED ASE**
```{r}
#i = 45
...
AR(2) Short Term
```{r}
shortAR <- fore.aruma.wge(us$hospitalizedCurrently, phi = est.us.diff.seasBIC$phi, n.ahead = 7, lastn = FALSE, limits = FALSE)
```

- ASE
```{r}
ASEshortAR = mean((us$hospitalizedCurrently[(124-7+1):124]-shortAR$f)^2)
ASEshortAR 
```
...
for( i in 1:(124-(trainingSize + horizon) + 1))
{
  forecasts = fore.arma.wge(us$hospitalizedCurrently[i:(i+(trainingSize-1))],phi = phis,n.ahead = horizon)
...
```{r}
i = 45
fs = fore.arma.wge(us$hospitalizedCurrently[i:(i+(trainingSize+horizon)-1)],phi = phis,n.ahead = 7, lastn = TRUE)
ASE = mean((us$hospitalizedCurrently[(i+trainingSize):(i+(trainingSize+horizon)-1)] - fs$f )^2)
ASE
```
...
AR(2) Long Term
```{r}
longAR <- fore.aruma.wge(us$hospitalizedCurrently, phi = est.us.diff.seasBIC$phi, n.ahead = 90, lastn = FALSE, limits = FALSE)
```
...
```{r}
#i = 45
#fs = fore.arma.wge(us$hospitalizedCurrently[i:(i+(trainingSize+horizon)-1)],phi = phis,n.ahead = 90, lastn = TRUE)
#ASE = mean((us$hospitalizedCurrently[(i+trainingSize):(i+(trainingSize+horizon)-1)] - fs$f )^2)
#ASE
```